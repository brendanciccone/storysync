...
```
   ...
   - **`fontAvailable: false` means the measurement describes a substituted typeface**, not the declared one — `fontFamily` is what the code asked for, not what rendered. Say so in the summary, and if Figma also lacks the font name it explicitly ("Figma has no *Inter*; install it or map it") rather than letting it surface as unexplained text drift. storysync cannot install fonts into Figma; that's manual in the desktop app.
