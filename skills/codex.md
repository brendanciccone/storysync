...
2. **REQUIRED — Get a deterministic styling spec.** For every component, run:
   ...
   It parses the source (CVA, hand-rolled `cn()`/`clsx()` conditionals, or inline classNames) and returns a per-variant styling object covering fill, text, border + borderColor + borderStyle, borderRadius, padding, fontFamily, fontSize, fontWeight, fontStyle, lineHeight, letterSpacing, textAlign, textTransform, textDecoration, shadow, gap, layout, alignItems, justifyContent, opacity — plus a `baseBindings` object and a `bindings` map per variant value. Each bound field (collection + token name) must be wired to the matching Figma variable from the Tokens step — components that look right in isolation but ignore theme/dark-mode changes mean the bindings were skipped. Unbound fields get the literal value. Also returns an `unresolved` array. Use the result verbatim — do not infer or guess. If `unresolved` is non-empty, ask the user to map each entry before any Figma write. If `inspect` fails, stop and report; do not fall back to manual source reading.
