...
6. Call `use_figma` per component. The instruction MUST embed the styling spec from step 4 verbatim — every variant value needs concrete fill, text color, padding, radius, border, font size, and shadow. Before each `use_figma` call, list the variant → property → resolved-value chain inline so the user can audit it. Bind fills/spacing/radius/typography/shadows to the variable collections from step 2 where possible.
