...
import { emitImagesBlock } from "./assets.js";

export interface PushScript {
  label: string;
  code: string;
  // Per-script warnings raised by the generator. Currently emitted by
  // §A.1's size guard when a script approaches the use_figma 50KB limit;
  // generatePushPlan aggregates these into the plan's top-level warnings.
  sizeWarnings?: string[];
}
...
export function generateComponentScript(input: ComponentInput): PushScript {
  ...
  // §1.3 — SVG icons collected into a per-script library so identical
  // icons (e.g. one chevron-down referenced in 17 Badge variants) share
  // a single outerHTML string. Populated by buildChildPayload via ctx.
  ...
  // §1.3 — SVG icon. createNodeFromSvg returns a vector group already
  // sized to the SVG's intrinsic dimensions; the optional resize syncs
  // to the rendered DOM size.
  lines.push(`    if (d.svr && typeof SVGS !== "undefined" && SVGS[d.svr]) {`);
  lines.push(`      const v = figma.createNodeFromSvg(SVGS[d.svr]);`);
  lines.push(`      if (d.w != null && d.h != null) { try { v.resize(d.w, d.h); } catch {} }`);
  lines.push(`      if (d.rot != null) { try { v.rotation = d.rot; } catch {} }`);
  lines.push(`      return v;`);
  lines.push(`    }`);
  ...
  // §1.7 — routed text props on the child text leaf.
  lines.push(`      if (d.tah) lb.textAlignHorizontal = d.tah;`);
  lines.push(`      if (d.ttc) lb.textCase = d.ttc;`);
  lines.push(`      if (d.ttd) lb.textDecoration = d.ttd;`);
  lines.push(`      if (d.tlh != null) lb.lineHeight = typeof d.tlh === "number" ? { unit: "PIXELS", value: d.tlh } : d.tlh;`);
  lines.push(`      if (d.tls != null) lb.letterSpacing = { unit: "PIXELS", value: d.tls };`);
  // §1.2 — wrap when the rendered text actually wrapped or when the
  // parent supplies a constrained width.
  lines.push(`      if (d.tar) lb.textAutoResize = d.tar;`);
  lines.push(`      if (d.tw != null) try { lb.resize(d.tw, lb.height); } catch {}`);
  lines.push(`      return lb;`);
  lines.push(`    }`);
  lines.push(`    const f = figma.createFrame();`);
  // §1.6 — VERTICAL is the safer default for block-flow children.
  // The browser-side capture already returns "row"/"column" when a
  // flex layout is detected; null falls into this branch.
  lines.push(`    f.layoutMode = d.lm || "VERTICAL";`);
  ...
  lines.push(`    if (d.im && typeof IMAGES !== "undefined" && IMAGES[d.im]) {`);
  lines.push(`      f.fills = [{ type: "IMAGE", scaleMode: "FILL", imageHash: IMAGES[d.im] }];`);
  lines.push(`    } else if (d.fg) {`);
  lines.push(`      const gs = d.fg.st.map((s) => ({ position: s[0], color: { r: s[1][0], g: s[1][1], b: s[1][2], a: s[1][3] != null ? s[1][3] : 1 } }));`);
  lines.push(`      const a = ((d.fg.a != null ? d.fg.a : 180) - 90) * Math.PI / 180;`);
  lines.push(`      const gradientTransform = [[Math.cos(a), Math.sin(a), 0.5 - Math.cos(a) * 0.5 - Math.sin(a) * 0.5], [-Math.sin(a), Math.cos(a), 0.5 + Math.sin(a) * 0.5 - Math.cos(a) * 0.5]];`);
  lines.push(`      f.fills = [{ type: d.fg.t === "L" ? "GRADIENT_LINEAR" : "GRADIENT_RADIAL", gradientStops: gs, gradientTransform }];`);
  lines.push(`    } else f.fills = fp ? [fp] : [];`);
  ...
  // §2.4 — strokeAlign INSIDE for web-accurate borders.
  lines.push(`    if (sp) { f.strokes = [sp]; f.strokeAlign = "INSIDE"; if (d.sw != null) f.strokeWeight = d.sw; if (d.sd) f.dashPattern = d.sd; }`);
  ...
  const payloads: VariantPayload[] = matrix.map((combo) => {
    ...
    // up in Figma instead of collapsing to a single styled box. SVG
    // icons land in `svgRegistry` so the script header can emit a
    // dedupe library.
    const kids = input.styling.renderedChildren?.[comboKey];
    if (kids && kids.length) {
      payload.kids = kids.map((k) => buildChildPayload(k, { svgRegistry }));
    }
    return payload;
  });

  // §1.3 — Emit the SVG library AHEAD of the VARIANTS array so the
  // apply loop's `SVGS[d.svr]` lookup resolves. Empty when no icons.
  if (svgRegistry.size > 0) {
    const svgObj: Record<string, string> = {};
    for (const [k, v] of svgRegistry.entries()) svgObj[k] = v;
    lines.push(`  // 4b) SVG icon library — referenced by child payloads via { svr: hash }.`);
    lines.push(`  const SVGS = ${JSON.stringify(svgObj)};`);
    lines.push("");
  } else {
    // Always define SVGS so the apply-loop's `typeof SVGS !== "undefined"`
    // guard doesn't fall into the no-op path when a payload mistakenly
    // carries `svr` without a registry entry.
    lines.push(`  const SVGS = {};`);
    lines.push("");
  }
  // §3.1 — Emit IMAGES block. Empty record still defines the variable
  // so the apply loop's `typeof IMAGES !== "undefined"` guard works.
  lines.push(`  // 4c) Image asset library — registers byte arrays once and exposes`);
  lines.push(`  //     their Figma image hashes for ImagePaint fills.`);
  lines.push(emitImagesBlock(input.styling.renderedImageAssets ?? {}));
  lines.push("");
  ...
  // §A.1 — Hard guard for the use_figma 50KB script limit. We warn at
  // 45KB so the user can switch to `--no-render` or filter components
  // before hitting the wall.
  const sizeWarnings: string[] = [];
  if (code.length > 45_000) {
    sizeWarnings.push(
      `${compName} script is ${(code.length / 1024).toFixed(1)} KB — close to use_figma's 50KB limit. Consider --no-render or filtering by --components.`,
    );
  }
  return {
...
interface VariantPayload {
  ...
  fg?: GradientPaintWire;             // gradient fill (§2.2)
  ...
  rot?: number;
  // §3.1 image fill on the variant root (rare; typical for image-only
  // stories like brand logos).
  im?: string;
  ...
  // §1.7 routed text properties (label arm)
  lah?: "LEFT" | "CENTER" | "RIGHT" | "JUSTIFIED";
  ltc?: "UPPER" | "LOWER" | "TITLE" | "ORIGINAL";
  ltd?: "UNDERLINE" | "STRIKETHROUGH" | "NONE";
  llh?: number | { unit: "PERCENT"; value: number };
  lls?: number;
  // §1.2 multi-line text wrapping (label-arm; rarely used since
  // single-label variant payloads are usually short, but available
  // for completeness)
  ltar?: "HEIGHT" | "WIDTH_AND_HEIGHT" | "NONE";
  ltw?: number;
...
interface ChildPayload {
  ...
  lp?: "ABSOLUTE";
  cn?: { h: "MIN" | "MAX" | "CENTER" | "SCALE" | "STRETCH"; v: "MIN" | "MAX" | "CENTER" | "SCALE" | "STRETCH" };
  x?: number;
  y?: number;
  // §3.2 rotation
  rot?: number;
  ...
  // §1.7 routed text properties (text-leaf arm)
  tah?: "LEFT" | "CENTER" | "RIGHT" | "JUSTIFIED";
  ttc?: "UPPER" | "LOWER" | "TITLE" | "ORIGINAL";
  ttd?: "UNDERLINE" | "STRIKETHROUGH" | "NONE";
  tlh?: number | { unit: "PERCENT"; value: number };
  tls?: number;
  // §1.2 multi-line wrapping
  tar?: "HEIGHT" | "WIDTH_AND_HEIGHT" | "NONE";
  tw?: number;
  // §1.3 SVG icon — references SVGS library by hash; apply loop calls
  // `figma.createNodeFromSvg(SVGS[d.svr])`.
  svr?: string;
  w?: number;
  h?: number;
  // §3.1 image fill — references the IMAGES map (hash → Figma image
  // hash). Apply loop emits an ImagePaint as the frame's first fill.
  im?: string;
  // Recursive
  kids?: ChildPayload[];
}

// Wire-format gradient — compact enough to fit the JSON payload while
// preserving the structure Figma's GradientPaint expects in the apply loop.
interface GradientPaintWire {
  t: "L" | "R";                       // type: linear / radial
  st: Array<[number, [number, number, number, number?]]>; // stops: [position, [r,g,b,a?]]
  a?: number;                         // angle in degrees (linear only)
}
...
const APPLY_LOOP: string[] = [
  ...
  `    if (d.im && typeof IMAGES !== "undefined" && IMAGES[d.im]) {`,
  `      c.fills = [{ type: "IMAGE", scaleMode: "FILL", imageHash: IMAGES[d.im] }];`,
  `    } else if (d.fg) {`,
  `      const gs = d.fg.st.map((s) => ({ position: s[0], color: { r: s[1][0], g: s[1][1], b: s[1][2], a: s[1][3] != null ? s[1][3] : 1 } }));`,
  `      const a = ((d.fg.a != null ? d.fg.a : 180) - 90) * Math.PI / 180;`,
  `      const gradientTransform = [[Math.cos(a), Math.sin(a), 0.5 - Math.cos(a) * 0.5 - Math.sin(a) * 0.5], [-Math.sin(a), Math.cos(a), 0.5 + Math.sin(a) * 0.5 - Math.cos(a) * 0.5]];`,
  `      c.fills = [{ type: d.fg.t === "L" ? "GRADIENT_LINEAR" : "GRADIENT_RADIAL", gradientStops: gs, gradientTransform }];`,
  `    } else c.fills = fp ? [fp] : [];`,
  `    // Stroke (border) — INSIDE align matches web box-sizing (§2.4).`,
  `    let sp = null;`,
  ...
  `    if (sp) { c.strokes = [sp]; c.strokeAlign = "INSIDE"; if (d.sw != null) c.strokeWeight = d.sw; if (d.sd) c.dashPattern = d.sd; }`,
  `    if (d.ef) c.effects = d.ef;`,
  `    if (d.op != null) c.opacity = d.op;`,
  `    if (d.cl) c.clipsContent = true;`,                                  // §2.3
  `    if (d.rot != null) try { c.rotation = d.rot; } catch {}`,             // §3.2
  ...
  `      if (lfp) lb.fills = [lfp];`,
  // §1.7 — route the new text props onto the label.
  `      if (d.lah) lb.textAlignHorizontal = d.lah;`,
  `      if (d.ltc) lb.textCase = d.ltc;`,
  `      if (d.ltd) lb.textDecoration = d.ltd;`,
  `      if (d.llh != null) lb.lineHeight = typeof d.llh === "number" ? { unit: "PIXELS", value: d.llh } : d.llh;`,
  `      if (d.lls != null) lb.letterSpacing = { unit: "PIXELS", value: d.lls };`,
  // §1.2 — label-arm multi-line wrap.
  `      if (d.ltar) lb.textAutoResize = d.ltar;`,
  `      if (d.ltw != null) try { lb.resize(d.ltw, lb.height); } catch {}`,
  `      c.appendChild(lb);`,
  `    }`,
  `    variants.push(c);`,
  `  }`,
...
function buildVariantPayload(
  ...
  // §1.6 — VERTICAL is the CSS block-flow default. Only override when
  // the source/runtime layer reports an explicit row/column layout.
  if (styling.layout) out.lm = styling.layout === "column" ? "VERTICAL" : "HORIZONTAL";
  else out.lm = "VERTICAL";
  ...
  // §2.2 gradient — emitted as the fill when present. Solid `fill` is
  // still attached for the binding path; gradient wins at apply-time.
  if (styling.gradient) {
    out.fg = gradientToWire(styling.gradient);
  }
  ...
  // §1.1 width/height — only emit when the author opted into the
  // constraint (CSS `width: Npx` literal, or `max-width: Npx` whose
  ...
  // §3.2 rotation
  if (styling.rotation != null && Math.abs(styling.rotation) > 0.5) out.rot = styling.rotation;
  // §3.1 image fill at variant root.
  if (styling.imageHash) out.im = styling.imageHash;
  ...
  // §1.7 — routed text properties (label arm).
  const align = mapTextAlign(styling.textAlign);
  if (align) out.lah = align;
  const tcase = mapTextCase(styling.textTransform);
  if (tcase) out.ltc = tcase;
  const tdec = mapTextDecoration(styling.textDecoration);
  if (tdec) out.ltd = tdec;
  const lh = parseLineHeight(styling.lineHeight, styling.fontSize);
  if (lh != null) out.llh = lh;
  const ls = parsePxNumber(styling.letterSpacing);
  if (ls != null) out.lls = ls;

  return out;
}

// §1.7 — Map ResolvedStyling text values to Figma's enum names.
function mapTextAlign(v?: string | null): VariantPayload["lah"] | undefined {
...
function mapTextCase(v?: string | null): VariantPayload["ltc"] | undefined {
...
function mapTextDecoration(v?: string | null): VariantPayload["ltd"] | undefined {
  if (v === "underline") return "UNDERLINE";
  if (v === "line-through") return "STRIKETHROUGH";
  // Figma has no overline; STRIKETHROUGH is closest, but we log via warning
  // since it's a fidelity loss for the rare cases that use it.
  if (v === "overline") return "STRIKETHROUGH";
...
function parseLineHeight(value?: string | null, _fontSize?: string | null): VariantPayload["llh"] | undefined {
  ...
  // Percent or unitless multiplier.
...
// §1.1 — Decide a FIXED width value when the source opted in (literal
// `width` or `max-width` rendered at the limit). Otherwise return
// null and let the apply loop fall back to HUG (AUTO).
function decideFixedWidth(styling: ResolvedStyling): number | null {
  if (styling.widthExplicit && styling.width != null && styling.width > 0) return styling.width;
  if (styling.maxWidth && styling.width != null) {
    const maxPx = parsePxNumber(styling.maxWidth);
    if (maxPx != null && Math.abs(styling.width - maxPx) <= 4) return maxPx;
  }
  return null;
}
function decideFixedHeight(styling: ResolvedStyling): number | null {
  if (styling.maxHeight && styling.height != null) {
    const maxPx = parsePxNumber(styling.maxHeight);
    if (maxPx != null && Math.abs(styling.height - maxPx) <= 4) return maxPx;
  }
  return null;
}

// §2.2 — Serialize a GradientPaintSpec into the compact wire form.
function gradientToWire(g: import("./inspect.js").GradientPaintSpec): GradientPaintWire {
  return {
    t: g.type === "GRADIENT_RADIAL" ? "R" : "L",
    a: g.angleDeg,
    st: g.stops.map((s) => {
      const t: [number, number, number, number?] = [
        Math.round(s.color.r * 10000) / 10000,
        Math.round(s.color.g * 10000) / 10000,
        Math.round(s.color.b * 10000) / 10000,
      ];
      if (s.color.a != null && s.color.a < 1) t[3] = Math.round(s.color.a * 10000) / 10000;
      return [Math.round(s.position * 10000) / 10000, t] as [number, [number, number, number, number?]];
    }),
  };
}
...
function buildChildPayload(child: RenderedChild, ctx?: { svgRegistry?: Map<string, string> }): ChildPayload {
  const styling = child.styling;
  const bindings: Bindings = child.bindings ?? {};
  // §1.3 — SVG leaf. Stash the markup in the script-level registry and
  // emit a reference so identical icons share a single entry.
  if (child.svgMarkup && ctx?.svgRegistry) {
    const hash = svgHash(child.svgMarkup);
    if (!ctx.svgRegistry.has(hash)) ctx.svgRegistry.set(hash, child.svgMarkup);
    const out: ChildPayload = { svr: hash };
    if (styling.width != null && styling.height != null) {
      out.w = styling.width;
      out.h = styling.height;
    }
    if (styling.rotation != null && Math.abs(styling.rotation) > 0.5) out.rot = styling.rotation;
    return out;
  }
  ...
  if (child.text && !child.children?.length) {
    ...
    // §1.7 — routed text props on the text leaf.
    const align = mapTextAlign(styling.textAlign);
    if (align) out.tah = align;
    const tcase = mapTextCase(styling.textTransform);
    if (tcase) out.ttc = tcase;
    const tdec = mapTextDecoration(styling.textDecoration);
    if (tdec) out.ttd = tdec;
    const lh = parseLineHeight(styling.lineHeight, styling.fontSize);
    if (lh != null) out.tlh = lh;
    const ls = parsePxNumber(styling.letterSpacing);
    if (ls != null) out.tls = ls;
    // §1.2 — wrap when the rendered DOM actually wrapped and we have
    // a parent inner width to constrain against.
    if (child.multiLine && child.parentInnerWidth != null) {
      out.tar = "HEIGHT";
      out.tw = child.parentInnerWidth;
    }
    return out;
  }
  // Container.
  const out: ChildPayload = {};
  // §1.6 — default VERTICAL for block flow (was HORIZONTAL).
  if (styling.layout) out.lm = styling.layout === "column" ? "VERTICAL" : "HORIZONTAL";
  else out.lm = "VERTICAL";
  const padding = parsePadding(styling.padding);
  ...
  if (radius != null) out.cr = radius;
  if (bindings.borderRadius) out.crb = [COLLECTION_META[bindings.borderRadius.collection].name, bindings.borderRadius.token];
  if (styling.gradient) out.fg = gradientToWire(styling.gradient);
  const fill = colorToTuple(styling.fill);
  if (fill) out.f = fill;
  if (bindings.fill) out.fb = [COLLECTION_META[bindings.fill.collection].name, bindings.fill.token];
  const strokeColor = styling.borderColor ?? extractBorderColor(styling.border);
  ...
  // §3.2 rotation
  if (styling.rotation != null && Math.abs(styling.rotation) > 0.5) out.rot = styling.rotation;
  // §3.1 image fill on the child frame.
  if (styling.imageHash) out.im = styling.imageHash;
  // §2.5 absolute-positioned children.
  if (styling.position === "absolute" || styling.position === "fixed") {
    out.lp = "ABSOLUTE";
    const cn = deriveConstraints(styling);
    out.cn = cn;
    const x = parsePxNumber(styling.leftOffset);
    const y = parsePxNumber(styling.topOffset);
    if (x != null) out.x = x;
    if (y != null) out.y = y;
  }
  ...
  if (child.text && !child.children?.length) {
    out.kids = [
      (() => {
        ...
        if (child.multiLine && child.parentInnerWidth != null) {
          leaf.tar = "HEIGHT";
          leaf.tw = child.parentInnerWidth;
        }
        return leaf;
      })(),
    ];
  } else if (child.children?.length) {
    out.kids = child.children.map((k) => buildChildPayload(k, ctx));
  }
  return out;
}

// §1.3 — Stable short hash for an SVG outerHTML string. Used as the
// key into the per-script SVGS library; collisions are astronomically
// unlikely within a single component's icon set.
function svgHash(s: string): string {
  let h = 5381;
  for (let i = 0; i < s.length; i++) h = ((h << 5) + h + s.charCodeAt(i)) | 0;
  return "s" + Math.abs(h).toString(36).padStart(6, "0");
}

// §3.3 — Parse `aspect-ratio: w / h` → numeric w/h ratio.
function parseAspectRatio(v: string): number | null {
...
// §2.5 — Derive Figma constraints from CSS position offsets. `top: 0`
// pins to top; `bottom: 0` pins to bottom; `top: 0, bottom: 0` stretches.
function deriveConstraints(styling: ResolvedStyling): { h: "MIN" | "MAX" | "CENTER" | "STRETCH"; v: "MIN" | "MAX" | "CENTER" | "STRETCH" } {
  const hasTop = styling.topOffset != null;
  const hasBottom = styling.bottomOffset != null;
  const hasLeft = styling.leftOffset != null;
  const hasRight = styling.rightOffset != null;
  const h: "MIN" | "MAX" | "CENTER" | "STRETCH" = hasLeft && hasRight ? "STRETCH" : hasRight && !hasLeft ? "MAX" : "MIN";
  const v: "MIN" | "MAX" | "CENTER" | "STRETCH" = hasTop && hasBottom ? "STRETCH" : hasBottom && !hasTop ? "MAX" : "MIN";
  return { h, v };
}
...
export function generatePushPlan(input: PlanInput): PushPlan {
  ...
  for (const c of input.components) {
    ...
    const script = generateComponentScript(c);
    scripts.push(script);
    // §A.1 — bubble per-script size warnings to the plan so the agent
    // surfaces them to the user before pushing.
    if (script.sizeWarnings?.length) warnings.push(...script.sizeWarnings);
  }
